...
def split_and_remove_nan(x, nan_indices=None):
    """
    ...
    split_up = np.split(x, nan_indices)
    
    # original code
    return [arr[~np.isnan(arr)] for arr in split_up if ~np.all(np.isnan(arr))]
    
    # my code
    # return [arr[~np.isnan(arr)] for arr in split_up if len(arr) > 0]

    

def debug_split(x):
    # Identify split points
    split_indices = np.where(np.isnan(x))[0]
    print(f"Split indices: {split_indices}")
    
    # Split based on NaN locations
    split_up = np.split(x, split_indices)
    print(f"Number of segments: {len(split_up)}")
    print(f"Segment sizes: {[len(arr) for arr in split_up]}")

    segment_sizes = [len(arr) for arr in split_up]
    segments_of_size_11500 = sum(1 for size in segment_sizes if size == 11501 or size == 11500)
    print(f"Number of segments with size 11500: {segments_of_size_11500}")
    
    # Filter segments and remove NaNs
    filtered = [arr[~np.isnan(arr)] for arr in split_up if ~np.all(np.isnan(arr))]
    print(f"Filtered segments: {len(filtered)}")
    print(f"Filtered sizes: {[len(arr) for arr in filtered]}")
    return filtered
...
def create_subjects(eeg_files, spindle_files, block_files):
    ...
    for idx, (eeg_file, spindle_file, block_file) in enumerate(zip(eeg_files, spindle_files, block_files)):
        ...
        # spindle_file = spindle_files[1]
        # block_file = block_files[1]

        eeg_data = loadmat(eeg_file)['EEGvector']
        spindle_vector = loadmat(spindle_file)['GCVect']

        # my code (there's one less block in spindle file of p1, so I changed eeg vector for now to avoid error, ask Karine)
        # nan_indices_eeg = np.where(np.isnan(eeg_data))[0]
        nan_indices_spindle = np.where(np.isnan(spindle_vector))[0]
        # set to nan if the spindle vector is nan
        eeg_data[nan_indices_spindle] = np.nan

        # split the data and spindle vectors into blocks
        data_vectors = split_and_remove_nan(eeg_data, nan_indices=None)
        spindle_vectors = split_and_remove_nan(spindle_vector, nan_indices=None)
